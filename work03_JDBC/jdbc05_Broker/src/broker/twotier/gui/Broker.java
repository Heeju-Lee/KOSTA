...
public class Broker implements ActionListener,ItemListener, Runnable{
	...
	*/
	...
	public static void main(String args[])throws Exception {
		...
		Thread t = new Thread(broker);//프로세스 안에서 돌기 때문에 인스턴스 넣어줌
		t.start();// 스타트만 해주면 자동적으로 thread 시작함
	}
	
	//쓰레드가 작업하는 코드를 구현하는 run()안에다 구현
	//3초마다 DB가서 상장된 주식의 정보를 가져오는 일만 전담하는 쓰레드를 작성...
	@Override
	public void run() {
		while(true) {//무한 루핑을 돌면서...3초마다 한번씩 디비의 모든 주식을 가져온다.
			try {
				showStockList(db.getAllStocks(),stockList);
				Thread.sleep(3000);
