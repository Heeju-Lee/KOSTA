...
public class MyBatisTestApp05 {
	...
	public static void main(String[] args) throws Exception{
		...
		user.setUserId("user05");//아이디를 변경...pk...새로운 user
		...
		//                                                    :: 주몽 ==> 온달 수정
		user.setUserName("온달");//user05...주몽--->온달
		System.out.println(":: 3. update(UPDATE)  ? "
													+ session.update("UserMapper05.updateUser",user));//1
		session.commit();
		
		//4. user05 아이디를 가진 사람을 삭제하는 단위테스트를 작서
		System.out.println(":: 4. remove(DELETE)  ? "+session.delete("UserMapper05.removeUser",user.getUserId()));
		session.commit();//반드시 commit 해줘야 함.
	}//end of main
}//end of class
