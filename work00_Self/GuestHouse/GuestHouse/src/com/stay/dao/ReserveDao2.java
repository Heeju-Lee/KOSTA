...
public class ReserveDao2 {
	...
	public static ReserveDao2 getInstance() {
		return dao;
	}
	...
	public 	ArrayList<GuestHouse> findByMinPrice(String location) throws DMLException {
		...
		String query = 	"SELECT g1.id, g1.name, g1.address, g1.room_num, g1.room_price, g1.capacity " +
				...
				"JOIN ( " +
					"SELECT distinct id, " +
							"FIRST_VALUE(room_num) OVER (PARTITION BY name ORDER BY room_price) Min_Room " +
					"FROM guesthouse " +
					"WHERE address = ? " +
				") g2 " +
				"ON g1.id = g2.id AND g1.room_num=g2.Min_Room " +
				"WHERE g1.address = ? ORDER BY g1.room_price";
	...
	public 	ArrayList<GuestHouse> findByTopTenPercent(String location) throws DMLException {
		...
		String query = "SELECT s.id, s.name, s.address, s.avg_rating " +
				"FROM ( " +
				// 서브쿼리: 특정 지역에서 각 숙소의 평균 평점과 백분위 순위를 계산
				"SELECT g1.id, g1.name, g1.address, g1.room_num, g1.room_price, g1.capacity, g2.avg_rating, " +
				// 지역별로 평균 평점의 백분위를 계산하여 상위 10%의 숙소를 추출
				"percent_rank() OVER (PARTITION BY g1.address ORDER BY g2.avg_rating DESC) AS percent " +
				"FROM guesthouse g1 " +
				// guesthouse와 review 테이블을 조인하여 평균 평점을 계산
				"JOIN ( " +
				"SELECT guesthouse_id, AVG(rating) AS avg_rating " +
				"FROM review " +
				"GROUP BY guesthouse_id " +
				") g2 " +
				"ON g1.id = g2.guesthouse_id " +
				"WHERE g1.address = ? " +
				") s " +
				// 최종 결과에서 상위 10%에 해당하는 숙소만 선택
				"WHERE s.percent <= 0.1 " +
				"group by s.id, s.name, s.address, s.avg_rating";
		...
		try (Connection con = getConnection();
			...
			throw new DMLException("[ERROR] 검색 도중 문제가 발생했습니다."+e.getMessage());
		}
	}
	...
	public 	ArrayList<GuestHouse> findByPrice(String location, int x, int y) throws DMLException {
		...
		String query = "SELECT id, name, address, room_num, room_price, capacity " +
						"FROM guesthouse " +
						"WHERE address = ? AND room_price between ? AND ? ORDER BY room_price";
