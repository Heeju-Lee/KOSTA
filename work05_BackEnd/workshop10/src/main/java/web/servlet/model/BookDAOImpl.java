...
public class BookDAOImpl implements BookDAO{
	...
	public String detailBook(String bookisbn) throws SQLException {
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
	        ...
	        if (rs.next()) {
	            Book book = new Book(rs.getString("isbn"), rs.getString("title"), rs.getString("catalogue"), rs.getString("nation"), 
	                    rs.getString("publish_date"), rs.getString("publisher"), rs.getString("author"), rs.getInt("price"), 
	                    rs.getString("description"));

	            return "Book 상세정보 출력 - 제목: " + book.getTitle() + ", 출판사: " + book.getPublisher() + ", 저자: " + book.getAuthor();
	        } else {
